...
plot(boot_est[[1]]$eval.grid,boot_est[[1]]$eval.gamma)
for(i in 2:Nsim){
  lines(boot_est$gamma.grid, boot_est[[i]]$eval.gamma)
  }
...
collect_parameters_boot = function(est.list, Nsim,level){
  ...
  sqrt.alpha.kappa.t.bs <- sapply(1:Nsim, function(i){
    est.list[[i]]$est_para$est$sqrt.alpha.kappa.t
  })
  ...
  save(est.bs,file = "Kerala_est_bs_1120.Rda")
  return(est.bs)
}
...
save(est_boot_data,file = "Kerala_est_boot_data_1120.Rda")
